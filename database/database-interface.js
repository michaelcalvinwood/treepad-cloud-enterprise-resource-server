...
exports.createTables = () => {
    ...
    server.dbPool.query(tc.createModulesTable, async (err, res, fields) => {
        if(err) console.error('Db error: ', err);
            ...
            await poolQuery(`INSERT INTO modules (module_id, module_name, icon, server, port) VALUES (1, 'Quill', '/svg/quill.svg', 'localhost', 7201)`);
            await poolQuery(`INSERT INTO modules (module_id, module_name, icon, server, port) VALUES (2, 'Image Gallery', '/svg/image-gallery.svg', 'localhost', 7202)`);
            await poolQuery(`INSERT INTO modules (module_id, module_name, icon, server, port) VALUES (3, 'Video Gallery', '/svg/video-gallery.svg', 'localhost', 7203)`);
            await poolQuery(`INSERT INTO modules (module_id, module_name, icon, server, port) VALUES (4, 'Audio Gallery', '/svg/audio-gallery.svg', 'localhost', 7204)`);
            await poolQuery(`INSERT INTO modules (module_id, module_name, icon, server, port) VALUES (5, 'Files', '/svg/files.svg', 'localhost', 7205)`);
            await poolQuery(`INSERT INTO modules (module_id, module_name, icon, server, port) VALUES (6, 'EtherPad', '/svg/etherpad.svg', 'localhost', 7206)`);
        }
    });
}
